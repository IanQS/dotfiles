...
%}
## Foundational Landscapes (The Soil)
...
## Structure & Core Logic (The Backbone)
...
## Personal Processing (The Active Fire)
...
- **MAGENTA**: Use magenta ONLY for the papers where you think: "I need to download this specific paper next because it holds the missing piece."
...
{%-
    set colorHeading = {
        ...
        "magenta": "🔍 Footprints (Key Citations to Steal)"
    }
%}

# 📜 Sequential Highlights (Reading Order)
...
{# --- LOOP THROUGH CHOSEN COLOR HEADINGS --- #}
{%- for color, heading in colorHeading -%}
{%- set colorEntries = newAnnotations | filterby("customColor", "startswith", color) -%}

{%- if colorEntries.length > 0 %}

### {{colorHeading[color]}}

{# --- SPECIAL HANDLING FOR GREY: SPLIT M: AND G: SUBS -- #}
{%- if color == "grey" -%}

#### 🧠 Universal Mechanisms & Laws

{% for entry in colorEntries -%}
{%- set annot = entry.annotation -%}
{%- if annot.comment and annot.comment.trim().startsWith("M:") %}

> [!quote|{{entry.customColor}}]+ {{calloutHeader(annot.type)}} ([page. {{annot.pageLabel}}](zotero://open-pdf/library/items/{{annot.attachment.itemKey}}?page={{annot.pageLabel}}&annotation={{annot.id}}))
> {{annot.annotatedText|replace("\n", " ")}}{% if annot.hashTags %} {{annot.hashTags}}{% endif %}
>
> - **Mechanism:** {{annot.comment | replace("M:", "") | trim | replace("\n", " ")}}

{%- endif -%}
{%- endfor %}

#### 🗺️ Circuit Geography & Nomenclature

{% for entry in colorEntries -%}
{%- set annot = entry.annotation -%}
{%- if annot.comment and annot.comment.trim().startsWith("G:") %}

> [!quote|{{entry.customColor}}]+ {{calloutHeader(annot.type)}} ([page. {{annot.pageLabel}}](zotero://open-pdf/library/items/{{annot.attachment.itemKey}}?page={{annot.pageLabel}}&annotation={{annot.id}}))
> {{annot.annotatedText|replace("\n", " ")}}{% if annot.hashTags %} {{annot.hashTags}}{% endif %}
>
> - **Geography:** {{annot.comment | replace("G:", "") | trim | replace("\n", " ")}}

{%- endif -%}
{%- endfor %}

#### 📝 Raw Definitions & General Context

{% for entry in colorEntries -%}
{%- set annot = entry.annotation -%}
{%- if not annot.comment or (not annot.comment.trim().startsWith("M:") and not annot.comment.trim().startsWith("G:")) %}

> [!quote|{{entry.customColor}}]+ {{calloutHeader(annot.type)}} ([page. {{annot.pageLabel}}](zotero://open-pdf/library/items/{{annot.attachment.itemKey}}?page={{annot.pageLabel}}&annotation={{annot.id}}))
> {{annot.annotatedText|replace("\n", " ")}}{% if annot.hashTags %} {{annot.hashTags}}{% endif %}
> {%- if annot.comment %}
>
> - {{annot.comment|replace("\n", " ")}}
>   {%- endif %}
...
{%- else -%}
{# --- STANDARD HANDLING FOR ALL OTHER COLORS --- #}
