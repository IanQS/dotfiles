...
// Hunk has no column cursor, so h/l scroll the code viewport horizontally instead.
const HORIZONTAL_COMMANDS: Readonly<Record<string, string>> = {
  h: "hunk.review.scrollCodeLeft",
  l: "hunk.review.scrollCodeRight",
};

/** Columns scrolled per h/l press, multiplied by any count prefix. */
const HORIZONTAL_STEP = 4;

/** Helix-style `g` motions; `gg` stays as the Vim jump to the start of the review. */
const GOTO_COMMANDS: Readonly<Record<string, { commandId: string; count?: number }>> = {
  g: { commandId: "hunk.review.jumpToTop" },
  e: { commandId: "hunk.review.jumpToBottom" },
  // Oversized counts clamp at the scroll bounds: column 0, or the end of the widest visible line.
  s: { commandId: "hunk.review.scrollCodeLeft", count: MAX_COUNT },
  h: { commandId: "hunk.review.scrollCodeLeft", count: MAX_COUNT },
  l: { commandId: "hunk.review.scrollCodeRight", count: MAX_COUNT },
};
...
export function createVimNavigationState(commands: VimNavigationCommands) {
  ...
  const executeRelative = (commandId: string, step = 1) => {
    const magnitude = Math.min(MAX_COUNT, count() * step);
    reset();
  ...
  const handleKey = (key: VimNavigationKey): VimNavigationResult => {
    ...
    if (prefix === "g") {
      const target = shiftedG ? undefined : GOTO_COMMANDS[text];
      reset();
      if (!target) return "pass";
      commands.execute(target.commandId, target.count ? { count: target.count } : undefined);
      return "handled";
    }
    ...
    const horizontal = HORIZONTAL_COMMANDS[text];
    if (horizontal) return executeRelative(horizontal, HORIZONTAL_STEP);
